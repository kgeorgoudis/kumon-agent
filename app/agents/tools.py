...
from app.domain.models import ChildProfile, MicroSkillId, ProgressReport, ProgressSuggestion, ProgressWorksheetPoint, SkillProgress
...
AccuracyTrend: TypeAlias = Literal["improving", "stable", "declining", "insufficient_data"]


def _classify_accuracy_trend(values: list[float], delta_threshold: float = 3.0) -> AccuracyTrend:
    if len(values) < 2:
...
def build_progress_fallback_suggestions(skill_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    if not skill_rows:
        return []
    weakest = min(skill_rows, key=lambda row: _float_from_row(row, "avg_accuracy_pct"))
    avg = _float_from_row(weakest, "avg_accuracy_pct")
    micro_skill_id = str(weakest.get("micro_skill_id", ""))
    worksheet_type = "mixed_review" if avg >= 90 else "drill" if avg >= 70 else "correction"
    return [
        ProgressSuggestion(
            target_micro_skill_id=micro_skill_id,
            suggested_worksheet_type=worksheet_type,
            rationale_el=f"Η '{micro_skill_id}' χρειάζεται το επόμενο μικρό βήμα εξάσκησης.",
            confidence="medium",
...
def build_manual_review_context(
    *,
    ...
    entry_details: list[dict[str, Any]] = details.get("entries", []) if isinstance(details, dict) else []
...
def build_next_step_plan_context(
    *,
    ...
    if isinstance(skill_rows, list) and skill_rows:
        typed_skill_rows = [cast(dict[str, object], row) for row in skill_rows if isinstance(row, dict)]
        if typed_skill_rows:
            weakest = min(typed_skill_rows, key=lambda row: _float_from_row(row, "avg_accuracy_pct"))
            micro_skill_id = weakest.get("micro_skill_id")
            if micro_skill_id is not None:
                weakest_skill_id = str(micro_skill_id)
    progression = (
